...
export declare type NominatimApiUrl<
  TEndpoint extends NominatimEndpoint,
  TReplacementUrl extends string | undefined = undefined,
> = `${TReplacementUrl extends string
  ? TReplacementUrl
  : NominatimBaseUrl}${TEndpoint}`;

export declare type ResponseOrError<TResponse extends object> =
  | TResponse
  | NominatimErrorResponse
  | NominatimStatusFormatErrorResponse;

export declare type NominatimEndpointParams<
  TEndpoint extends NominatimEndpoint,
> = TEndpoint extends "/search"
  ? NominatimSearchParams
  : TEndpoint extends "/reverse"
  ? NominatimReverseParams
  : TEndpoint extends "/lookup"
  ? NominatimLookupParams
  : TEndpoint extends "/status.php"
  ? NominatimStatusParams
  : never;

export type NominatimEndpointResponse<
  TEndpoint extends NominatimEndpoint,
  TEndpointParams extends NominatimEndpointParams<TEndpoint>,
> = TEndpointParams extends NominatimSearchParams
  ? NominatimSearchResponse<TEndpointParams>
  : TEndpointParams extends NominatimReverseParams
  ? NominatimReverseResponse<TEndpointParams>
  : TEndpointParams extends NominatimLookupParams
  ? NominatimLookupResponse<TEndpointParams>
  : TEndpointParams extends NominatimStatusParams
  ? NominatimStatusResponse<TEndpointParams>
  : never;
